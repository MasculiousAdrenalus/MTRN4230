...
function path = z3424655_Gossling_Bryce_ImageProcessing()
    ...
    [centers_green, radii_green, metric_green] = imfindcircles(bw_green,[16 80],'ObjectPolarity','bright');
    START = centers_red
    ...
    plot(centers(:,1),centers(:,2),'*r', 'LineWidth',5);
    hold off;
    %% FIND PATH
    path = [START];
    temp = [centers; FINISH];
    ...
    hold on;
    plot(path(:,1),path(:,2),'-y', 'LineWidth',5);
    hold off;
end
%% get distance func
