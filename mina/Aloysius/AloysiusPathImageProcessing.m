...
function path_ret = AloysiusPathImageProcessing(image_name_string)
    close all;
    clear all;
    clc;

    if ~exist('image_name_string')
        image_name_string = 'Tue1200.jpg';
    end 
    image1 = image_name_string;
    board = imread(image1);
...
channel3Max = 102.000;
    ...
    title('Points')
    hold on;
    plot(unique_centers(:,1),unique_centers(:,2),'*b','LineWidth',5);
    plot(start_point(:,1),start_point(:,2),'*r','LineWidth',5);
    plot(end_point(:,1),end_point(:,2),'*g','LineWidth',5);
    ...
    imshow(board)
    hold on;
    plot(path(:,1),path(:,2),'-g','LineWidth',10);
    plot(start_point(:,1),start_point(:,2),'*r','LineWidth',5);
