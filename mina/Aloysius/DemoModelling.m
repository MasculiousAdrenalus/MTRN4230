...
%% q1 DH parameter table 
% A Link object holds information related to robot joints and links 
% [THETA D A ALPHA SIGMA OFFSET] SIGMA=0 revolute, 1 prismatic joint
...
% https://au.mathworks.com/help/vision/ref/plotcamera.html?fbclid=IwAR3DE-zHe5-T0SNgLJboGSLNTNP5PvEvapdcZZNbp1zhF4w4EpMcb5xyWdo
...
%Plot the extent of the reachability of the arm on the surface of the table, 
% assuming the end effector is facing vertically down and touching the table

% Demo: 
% - Draw a top view of the table including the line showing the extent of the reachability of the arm 
% - calculate the radius of the end effector about the robot origin.
...
% Simulate the motion of the Cartesian kinematic trajectory in RVCTools from a path image 
% which will be supplied during your tutorial, showing the entire path of the end effector 
% in 3D during the animation
% a. (2 points) Demo: Demonstrate this path to your tutor.

% put image name 'image_path.jpg' as an argument 


my_traj_path = AloysiusPathImageProcessing(); % path in pixel locations 
...
for i = 1:length(my_traj_path)-1
    ...
    % position 1
    pos1 = convertPixelXYToRobotXY(my_traj_path(i,1),my_traj_path(i,2));
    ...
    T1(1:3, 4) =  posXYZ1';
    % position 2
    pos2 = convertPixelXYToRobotXY(my_traj_path(i+1,1),my_traj_path(i+1,2));
    % gives the required position 
